...
public class HashMapLinkedListUnordered<K, V> implements Cloneable {
    ...
    public LinkedListUnordered<K> keySet() {
        LinkedListUnordered<K> keys = new LinkedListUnordered<>();
        for (int i = 0; i < capacity; i++) {
            LinkedListUnordered<Node> bucket = table[i];
            LinkedListUnordered<Node>.Node current = bucket.getPrimeiro();
            while (current != null) {
                keys.addLast(current.elemento.key);
                current = current.proximo;
            }
        }
        return keys;
     ...
     *
     * @param original O data_structures.HashMap original a ser copiado.
     */
    @SuppressWarnings("unchecked")
    public HashMapLinkedListUnordered(HashMapLinkedListUnordered<K, V> original) {
        if (original == null) throw new IllegalArgumentException("data_structures.HashMap original não pode ser nulo");
    ...
    /**
     * Metodo clone para criar uma cópia profunda do data_structures.HashMap.
     *
